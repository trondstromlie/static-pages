...
1. **Stå i rota først:** `cd "$(git rev-parse --show-toplevel)"`. Repoet ligger ofte i en undermappe, så mappa du starter i er ikke nødvendigvis rota.
2. Brukeren kan ikke git. **Du** gjør alt git-arbeidet, uten å be dem om noe.
3. Brukeren jobber **kun i sin egen mappe** (`trond/`, `marlene/`, …). Aldri rør andres.
4. Aldri lagre noe rett på `main` — alltid arbeidskopi → pull request → merge.
5. Når de er ferdige: legg det ut, sjekk for konflikter, merge, **vent til URL-en svarer 200**, og **gi dem lenka** så de kan dele den med én gang.
